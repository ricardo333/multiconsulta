...
peticiones.cargandoPeticionPrincipal = function cargandoPeticionPrincipal()
{
        ...
        $("#contencionLlamadasGrafico").html("");	
        $("#preloadGraph").html(`<div id="carga_person" class="pre-estados-modems">
                                <div class="loader">Loading...</div>
                              </div>`);

        $.ajax({
              url:"/administrador/contencion-llamadas/grafico-llamadas-contenidas",
              method:"post",
              //dataType: "json",
              cache: false, 
        })
        .done(function(data){
                ...
                let colores = data.response.colorContencionLlamadas
                let estado = data.response.estado

                let resultContencion = data.response.resultHoraTotalContencion
                let ok = resultContencion[0].total - resultContencion[0].contencion
                
                if(estado){
                  
                  let chartDataHisTroba = [
                          { name:"Alarmado", data:[],color: colores[0].color},
                          { name:"NoAlarmado", data:[],color: colores[1].color},
                          { name:"NoExiste", data:[] ,color: colores[2].color},
                          { name:"Total", data:[],color: colores[3].color}
                  ];
                  ...
                  grafico_data.forEach(el => {
                          chartDataHisTroba[0].data.push([el.hora,parseFloat(el.Alertado)]);
                          chartDataHisTroba[1].data.push([el.hora,parseFloat(el.NoAlarmado)]);
                          chartDataHisTroba[2].data.push([el.hora,parseFloat(el.NoExiste)]);
                          chartDataHisTroba[3].data.push([el.hora,parseFloat(el.Total)]);
                  ...
                  Highcharts.chart("contencionLlamadasGrafico", {
                          title: {
                                  text: `CENTRO DE CONTROL MOVISTAR1`
                          },
                                                    subtitle: {
                                                      text: `<span>${resultContencion[0].total} Total de Llamadas DMPE</span><br> <span>${ok} No Alarmadas</span><br> <span> ${resultContencion[0].contencion} Llamadas contenidas</span>`
                                                    },
                                                    xAxis: {
                  ...
                  $("#contencionLlamadasGrafico").html(`<div class="container">
