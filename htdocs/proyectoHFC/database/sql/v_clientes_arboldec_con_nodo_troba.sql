...
DROP VIEW IF EXISTS `v_clientes_arboldec_con_nodo_troba`$$

CREATE ALGORITHM=UNDEFINED DEFINER=`rfalla`@`%` SQL SECURITY DEFINER VIEW `arboldecisiones`.`v_clientes_arboldec_con_nodo_troba` AS 
-- Obteniendo Nodo y Troba para clientes HFC, obtiendo datos de planta clarita si es necesario
SELECT DISTINCT dh1.idclientecrm, IF(nc1.NODO IS NULL OR nc1.NODO='', pc1.nodo, nc1.NODO) AS nodo_real , IF(nc1.TROBA IS NULL OR nc1.TROBA='', pc1.plano, nc1.TROBA) AS troba_real
...
SELECT DISTINCT dh2.idclientecrm, pc2.nodo AS nodo_real , pc2.plano AS troba_real
